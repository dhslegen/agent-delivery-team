...
- 封盘命令：`node bin/baseline.mjs --lock` → 生成 `baseline/baseline.locked.json`
...
hooks → ~/.claude/delivery-metrics/events.jsonl
      → bin/aggregate.mjs → metrics.db（SQLite）
...
events.jsonl 字段结构：
```json
{"event":"session_start","ts":"2026-01-01T00:00:00Z","session_id":"<uuid>","project_id":"<id>","cwd":"<path>"}
{"event":"session_end","ts":"2026-01-01T01:00:00Z","session_id":"<uuid>","project_id":"<id>"}
{"event":"tool_use","ts":"2026-01-01T00:10:00Z","tool":"Write","project_id":"<id>","session_id":"<uuid>"}
```
