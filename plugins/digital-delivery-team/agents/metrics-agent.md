...
- `baseline/baseline.locked.json`（封盘基线，必读；缺失则停止并提示先跑 `node bin/baseline.mjs --lock`）
...
4. 必须附原始数据来源链接（events.jsonl 路径 / metrics.db 路径）
...
- `baseline/baseline.locked.json` 缺失 → 停止 → 提示先跑 `node bin/baseline.mjs --lock`
- `docs/efficiency-report.raw.md` 缺失 → 停止 → 提示先跑 `node bin/report.mjs`
