...
/kickoff
# 产物：docs/prd.md、docs/wbs.md、docs/api-contract.yaml、docs/data-model.md
...
# 6. 打包 + 效率报告
/ship
# 产物：docs/delivery-package/（README + deploy + demo）、docs/efficiency-report.raw.md
```
...
# 单独补充架构设计
/design
# 产物：docs/api-contract.yaml、docs/data-model.md、docs/architecture.md
...
node bin/aggregate.mjs

# 产出报告（指定项目 ID）
node bin/report.mjs --project proj-your-id --out docs/my-report.md
...
**Q: 度量数据在哪里？**  
A: 默认在 `~/.claude/delivery-metrics/`。可通过 `DELIVERY_METRICS_DIR` 环境变量修改路径。

**Q: 可以只用部分命令不用全套吗？**  
A: 完全可以，每个岗位命令都是独立可运行的，无强制依赖顺序。

**Q: Node.js 版本不够怎么办？**  
A: 需要 ≥ 22.0.0（`node:sqlite` 内置 API）。使用 `nvm install 22` 或 `brew install node` 升级。
